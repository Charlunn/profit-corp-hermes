...
class TemplateContractTests(unittest.TestCase):
    ...
    def test_protected_and_safe_layers_and_verification_commands_are_explicit(self) -> None:
        ...
        self.assertIn("single shared Supabase", content)
        self.assertIn("Independent backend bootstrap", content)
        ...
        self.assertIn("src/lib/supabase-browser.ts", content)
        self.assertIn("src/lib/supabase-server.ts", content)
        ...
        self.assertIn(".hermes/shared-backend-guardrails.json", content)
        ...
        self.assertIn("must not directly mutate `users`, `orders`, `payments`, or `subscriptions`", content)
        self.assertIn("python -m unittest tests.test_template_registry tests.test_template_contract", content)
        self.assertIn("python -m unittest tests.test_instantiate_template_project", content)
        self.assertIn("python -m unittest tests.test_check_template_conformance", content)
