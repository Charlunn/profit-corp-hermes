...
6. Decision Package 产物层
   ...
   - 运行 `python scripts/generate_operating_visibility.py`
   - 读取 `assets/shared/visibility/OPERATING_VISIBILITY.md`
   - 确认三类产物都来自同一天主包派生，且保留到 prioritized shortlist、role outputs、CEO synthesis 的证据回链
   - 确认可见性视图明确标记 `HEALTHY`、`WATCH` 或 `ACTION REQUIRED`，并把 blocked / pending / failed / stale 例外放到顶部告警区
7. Accountant 审计阶段
...
- 默认中文输出。
- 任何风险结论必须带证据（文件或数字）。
- 主决策包、执行包、董事会简报、可见性视图必须来自同一天主包派生，不得各自回读 raw signals。
- 关键风险、机会、下一步必须保留 evidence backlink，能沿着主包回链到上游证据链。
- 可见性视图必须保持 calm-by-default：健康时输出紧凑摘要；存在 blocked / pending / failed / stale 时，顶部告警区优先展示。
- 可见性视图只允许 Top 3 next actions，不得变成 backlog、queue 或 dashboard。
- 如果所有指标健康，摘要第一行写“Daily pipeline completed: HEALTHY”。
- 如果存在待观察但未阻塞的问题，摘要第一行写“Daily pipeline completed: WATCH”。
- 如果存在重大风险或阻塞，摘要第一行写“Daily pipeline completed: ACTION REQUIRED”。
