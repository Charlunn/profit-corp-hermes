...
def build_status_markdown(events: list[dict[str, Any]]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- **Vercel Link Status**: `{vercel.get('linked', 'not available')}`",
            f"- **Vercel Env Contract**: `{vercel.get('env_contract_path', vercel.get('env_contract', {}).get('evidence_path', 'not available') if isinstance(vercel.get('env_contract'), dict) else 'not available')}`",
            f"- **Deploy URL**: `{vercel.get('deploy_url', vercel.get('deployment_url', 'not available'))}`",
